...
require(ggplot2)
require(png)
df <- read.csv("scrape_from_scratch.csv")
```
...
| Support Vector Machine (dup/base) |             |
| Support Vector Machine (dup/hard) |             |
...
From the table, it is evident that using the base features the models returned by each method performed extremely well (over 90% accuracy) even with duplication applied. Whereas, once hard mode was applied, the accuracy dropped significantly regardless if the duplication method was applied or not, indicating that type_2 and damage_from_{Type} features add valuable information in predicting the Pokemon's type.
