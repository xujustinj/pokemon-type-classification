...
Pokémon is one of the most popular games worldwide. In it, players catch, train, and battle using monsters known as Pokemon. In particular, each Pokemon has one or two types such as elemntal type "Water" or abstract type "Fighting". Each type has advantages and disadvantages against others in battle. As a result, Pokemon players share great interest in using effective types to win battles and have interest in predicting a Pokemon's type using other attributes. However, many of the previous attempts have failed as people have a limited number of observations due to game release periods or used limited attributes by choice. Thus, we continue their studies with an improved data set and a more diverse feature space and use classification techniques to re-examine the relationship between Pokemon types and their other attributes. More importantly, we compare the performance between different models through nested CV and additionally, try to predict Pokemon's secondary type if it exists.
...
* [@pokemontypepredstats]: Multi-label classification using MultiLabelBinarizer, basically one-hot encoding the 18 types and thus allowing it to predict more than one class. Another approach used is classifier chaining and label powersets. The data used only includes the stats of each Pokemon.
...
Overall each classifier performed extremely well (over 90% accuracy) when `type_2` and `damage_from_{TYPE}` were included in the feature space. This result is not entirely surprising since these features provide a ton of additional information that is evidently very useful when trying to predict the primary type. As discussed in the introduction, certain types of pokemon have determined weaknesses to other specfic types in terms of damage taken. Additionally, since the `damage_from_{TYPE}` variables would effectively eliminate some of the possible types, the `type_2` variable would provide even more information about the possible values that `type_1` can take on. Thus, the performance of the models are not unexpected but very welcome. 

With this information, we move onto investigate how the same models will perform without the `type_2` and `damage_from_{TYPE}`. From the confusion matrix plots using `hardmode` (without `type_2` and `damage_from_{TYPE}`), the performance and accuracy of the models evidently drop by a significant factor. For instance, in the Confusion Matrix for Random Forest (Hard) by true label plot, we observe that the model was not able to predict any flying types correctly and did extremely poorly for fairy types. This is likely due to a small sample size for certain types, for example there exists a sparcity of primary flying type (only 8 observations) and fairy type pokemon in our dataset. Another interesting note is that a lot of pokemon get their primary type as water while actually belonging to another type by the random forest model, this could be potentially due to the colour of the sprites just so happening to be more blue. 

The confusion matrix plots using `hardmode `for each model also allow us to constrast between primary types. For instance using the random forest model, a significant portion of primary ice and poison type pokemon get predicted as water.
...
```{r, out.width = "500px", echo = F, fig.align='center'}
knitr::include_graphics("models/random_forest_hard/confusion_matrix_by_pred.png")
```
